...
export default function Home({ slug }: { slug: string }) {
  ...
  const openPhotoPuzzle = () => { if (!puzzlePhoto) return; setPuzzleOrder(makePuzzleOrder()); setSelectedPuzzleTile(null); setPuzzleOpened(true); };
  ...
  const openMemory = (event: React.MouseEvent<HTMLButtonElement>) => { if (features.ambientHeartsEnabled) launchHeart(event); if (features.celebrationOnOpenEnabled) { setCelebrationActive(true); window.setTimeout(() => setCelebrationActive(false), 1_650); } void startMusicAfterWelcome(); setRevealDismissed(true); };
