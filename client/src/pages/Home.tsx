...
import { BookHeart, CheckCircle2, ChevronLeft, ChevronRight, Expand, Facebook, Heart, Instagram, MailOpen, MapPin, Pause, Play, Puzzle, QrCode, RotateCcw, Send, Share2, Shuffle, Sparkles, Video, X } from "lucide-react";
...
import { playEffectSound, type EffectSound } from "@/soundEffects";
...
export default function Home({ slug }: { slug: string }) {
  ...
  const [puzzleOpened, setPuzzleOpened] = useState(false);
  ...
  const [flippedPhotoId, setFlippedPhotoId] = useState<number | null>(null);
  const [secretGiftOpened, setSecretGiftOpened] = useState(false);
  const [easterEggOpened, setEasterEggOpened] = useState(false);
  ...
  const puzzlePhoto = photos.find((photo) => photo.id === features.puzzleImageId) ?? photos[0];
  const makePuzzleOrder = () => {
    const order = Array.from({ length: 9 }, (_, index) => index);
    for (let index = order.length - 1; index > 0; index -= 1) {
      const swapIndex = Math.floor(Math.random() * (index + 1));
      [order[index], order[swapIndex]] = [order[swapIndex], order[index]];
    }
    return order.every((tile, index) => tile === index) ? [1, 0, 2, 3, 4, 5, 6, 7, 8] : order;
  };
  const openPhotoPuzzle = () => { if (!puzzlePhoto) return; playSound("puzzle"); setPuzzleOrder(makePuzzleOrder()); setSelectedPuzzleTile(null); setPuzzleOpened(true); };
  const choosePuzzleTile = (position: number) => {
    if (selectedPuzzleTile === null) { setSelectedPuzzleTile(position); return; }
    if (selectedPuzzleTile === position) { setSelectedPuzzleTile(null); return; }
    setPuzzleOrder((order) => order.map((tile, index) => index === selectedPuzzleTile ? order[position] : index === position ? order[selectedPuzzleTile] : tile));
    setSelectedPuzzleTile(null);
  };
  const puzzleSolved = puzzleOrder.length === 9 && puzzleOrder.every((tile, index) => tile === index);
  ...
  const playSound = (effect: EffectSound) => { if (features.soundEffectsEnabled) playEffectSound(effect, features.soundEffectsVolume); };
  ...
  const openMemory = (event: React.MouseEvent<HTMLButtonElement>) => { playSound("open"); if (features.ambientHeartsEnabled) launchHeart(event); if (features.celebrationOnOpenEnabled) { setCelebrationActive(true); window.setTimeout(() => setCelebrationActive(false), 1_650); } void startMusicAfterWelcome(); setRevealDismissed(true); };
  ...
  return <div className={`legacy-anniversary legacy-layout-${uiLayout.id} min-h-screen legacy-bg-${features.backgroundStyle} legacy-visual-${visualTheme.id} ${customFontFace ? "legacy-font-custom" : `legacy-font-${features.fontFamily}`} ${nightMode ? "legacy-night" : ""} ${features.sparklesEnabled ? "legacy-delight-sparkles" : ""} ${features.filmOverlayEnabled ? "legacy-delight-film" : ""}`} style={themeStyle}>
    ...
    {features.filmOverlayEnabled && <div className="legacy-film-overlay" aria-hidden="true" />}
    ...
    {unlocked && <main className="legacy-main-content">
      <header className="legacy-header"><h1>{site.site.title}</h1>{features.welcomeTitle && <p className="legacy-welcome-title">{features.welcomeTitle}</p>}{features.welcomeMessage && <p className="legacy-welcome-message">{features.welcomeMessage}</p>}<div className="legacy-header-delights">{features.ambientHeartsEnabled && <button type="button" className="legacy-heart-button" onClick={(event) => { playSound("heart"); launchHeart(event); }}><Heart size={16} fill="currentColor" />ส่งหัวใจ</button>}{features.easterEggEnabled && <button type="button" className="legacy-easter-egg" onClick={() => { playSound("secret"); setEasterEggOpened(true); }} aria-label="ค้นหาข้อความลับ"><Sparkles size={16} /></button>}</div><div className="legacy-clock"><ClockBox value={clock.days} label="วัน" /><ClockBox value={clock.hours} label="ชั่วโมง" /><ClockBox value={clock.minutes} label="นาที" /><ClockBox value={clock.seconds} label="วินาที" /></div></header>
      {features.secretGiftEnabled && <section className="legacy-section legacy-secret-gift-section"><button type="button" className="legacy-secret-gift" onClick={() => { playSound("gift"); setSecretGiftOpened(true); }}><span><Sparkles size={19} /></span><strong>{features.secretGiftTitle || "กล่องของขวัญลับ"}</strong><small>กดเพื่อเปิดเซอร์ไพรส์</small></button></section>}
      ...
      {!features.hideGallery && <section className="legacy-section legacy-gallery-section"><h3>รูปที่คบกัน 📸</h3><p className="legacy-section-hint">แตะรูปเพื่อพลิกอ่านข้อความ แล้วกดปุ่มมุมขวาเพื่อเปิดรูปเต็มจอ</p><div className="legacy-photo-slider"><button type="button" className="legacy-arrow legacy-photo-prev" onClick={() => movePhoto(-1)} disabled={!photos.length} aria-label="รูปก่อนหน้า"><ChevronLeft size={22} /></button><div className="legacy-photo-viewport"><div className="legacy-photo-track" style={{ transform: `translateX(-${photoIndex * 100}%)` }}>{photos.length ? photos.map((asset) => <div className="legacy-photo-slide" key={asset.id}><div className={`legacy-flip-photo-card ${flippedPhotoId === asset.id ? "is-flipped" : ""}`}><button type="button" className="legacy-flip-photo-toggle" onClick={() => { playSound("flip"); setFlippedPhotoId((id) => id === asset.id ? null : asset.id); }} aria-label={flippedPhotoId === asset.id ? `ดูรูป ${asset.originalName} อีกครั้ง` : `พลิกเพื่ออ่านข้อความของรูป ${asset.originalName}`} aria-pressed={flippedPhotoId === asset.id}><span className="legacy-flip-photo-inner"><span className="legacy-flip-photo-face legacy-flip-photo-front"><img src={asset.url} alt={asset.caption || asset.originalName} /><span className="legacy-flip-photo-hint"><MailOpen size={15} />แตะเพื่ออ่านข้อความ</span></span><span className="legacy-flip-photo-face legacy-flip-photo-back"><MailOpen size={25} /><strong>ความทรงจำของเรา</strong><p>{asset.caption || "รูปนี้ยังไม่มีข้อความกำกับ ลองเพิ่มข้อความจากหลังบ้านได้เลย"}</p><small>แตะอีกครั้งเพื่อพลิกกลับ</small></span></span></button><button type="button" className="legacy-photo-fullscreen" onClick={() => setSelectedPhoto({ url: asset.url, originalName: asset.originalName, caption: asset.caption ?? "" })} aria-label={`เปิดรูป ${asset.originalName} เต็มจอ`}><Expand size={17} /><span>เต็มจอ</span></button></div></div>) : <div className="legacy-photo-slide"><div className="legacy-empty-photo">ยังไม่มีรูปภาพ</div></div>}</div></div><button type="button" className="legacy-arrow legacy-photo-next" onClick={() => movePhoto(1)} disabled={!photos.length} aria-label="รูปถัดไป"><ChevronRight size={22} /></button></div></section>}
    ...
    {unlocked && !revealDismissed && <section className="legacy-reveal-overlay" role="dialog" aria-modal="true" aria-label="ข้อความเปิดตัว"><div className="legacy-reveal-orb legacy-reveal-orb-one" /><div className="legacy-reveal-orb legacy-reveal-orb-two" /><div className="legacy-reveal-card legacy-welcome-envelope"><div className="legacy-reveal-hearts" aria-hidden="true"><span>♥</span><span>♥</span><span>♥</span></div><div className="legacy-envelope-flap" aria-hidden="true" /><p className="legacy-reveal-eyebrow">A little surprise for you</p>{puzzlePhoto && <img className="legacy-welcome-photo" src={puzzlePhoto.url} alt={puzzlePhoto.caption || "รูปแฟนสำหรับจิ๊กซอ"} />}<h2>{revealContent.headline}</h2><p className="legacy-typed-message">{revealContent.message}</p><div className="legacy-reveal-actions">{puzzlePhoto && <button type="button" className="legacy-puzzle-open" onClick={openPhotoPuzzle}><Puzzle size={17} />เล่นจิ๊กซอรูปนี้</button>}<button type="button" onClick={openMemory}><Heart size={17} fill="currentColor" />เปิดความทรงจำ</button></div></div></section>}
    {unlocked && puzzleOpened && puzzlePhoto && <section className="legacy-puzzle-modal" role="dialog" aria-modal="true" aria-label="จิ๊กซอรูปความทรงจำ"><button type="button" className="legacy-memory-modal-backdrop" onClick={() => setPuzzleOpened(false)} aria-label="ปิดจิ๊กซอ" /><article><button type="button" className="legacy-memory-modal-close" onClick={() => setPuzzleOpened(false)} aria-label="ปิด"><X size={19} /></button><Puzzle size={27} /><p className="legacy-question-eyebrow">Photo puzzle</p><h3>{puzzleSolved ? "ต่อรูปสำเร็จแล้ว!" : "ต่อจิ๊กซอความทรงจำ"}</h3><p>{puzzleSolved ? "เก่งมากเลย เรามาเปิดความทรงจำกันนะ" : "แตะชิ้นส่วน 2 ชิ้นเพื่อสลับตำแหน่งให้เป็นรูปที่สมบูรณ์"}</p><div className="legacy-photo-puzzle" aria-label="จิ๊กซอรูปภาพ">{puzzleOrder.map((tile, position) => <button key={`${tile}-${position}`} type="button" onClick={() => choosePuzzleTile(position)} aria-pressed={selectedPuzzleTile === position} className={selectedPuzzleTile === position ? "selected" : ""} style={{ backgroundImage: `url(${puzzlePhoto.url})`, backgroundPosition: `${(tile % 3) * 50}% ${Math.floor(tile / 3) * 50}%` }}><span className="sr-only">ชิ้นส่วน {position + 1}</span></button>)}</div>{puzzleSolved ? <button type="button" className="legacy-puzzle-finish" onClick={openMemory}><CheckCircle2 size={17} />เปิดความทรงจำ</button> : <button type="button" className="legacy-puzzle-reset" onClick={openPhotoPuzzle}><Shuffle size={16} />สลับใหม่</button>}</article></section>}
    ...
    {unlocked && secretGiftOpened && <section className="legacy-memory-modal legacy-delight-modal" role="dialog" aria-modal="true" aria-label="กล่องของขวัญลับ"><button type="button" className="legacy-memory-modal-backdrop" onClick={() => setSecretGiftOpened(false)} aria-label="ปิดกล่องของขวัญ" /><article><button type="button" className="legacy-memory-modal-close" onClick={() => setSecretGiftOpened(false)} aria-label="ปิด"><X size={19} /></button><Sparkles size={29} /><p className="legacy-question-eyebrow">A little gift for you</p><h3>{features.secretGiftTitle || "กล่องของขวัญลับ"}</h3><p>{features.secretGiftMessage || "ขอบคุณที่เข้ามาในความทรงจำนี้นะ 💝"}</p></article></section>}
    {unlocked && easterEggOpened && <section className="legacy-memory-modal legacy-delight-modal" role="dialog" aria-modal="true" aria-label="ข้อความลับ"><button type="button" className="legacy-memory-modal-backdrop" onClick={() => setEasterEggOpened(false)} aria-label="ปิดข้อความลับ" /><article><button type="button" className="legacy-memory-modal-close" onClick={() => setEasterEggOpened(false)} aria-label="ปิด"><X size={19} /></button><Sparkles size={29} /><p className="legacy-question-eyebrow">You found it</p><h3>เจอข้อความลับแล้ว</h3><p>{features.welcomeMessage || site.settings.memoryMessage || "ขอบคุณที่ตั้งใจตามหาความลับของเรา ✨"}</p></article></section>}
