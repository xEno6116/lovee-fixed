...
export default function Home({ slug }: { slug: string }) {
  ...
  const openQuestionLetter = () => { questionOpenedAt.current = Date.now(); setQuestionOpened(true); setQuestionStatus(""); };
  const submitQuestion = () => { if (!questionAnswer.trim() || !window.confirm("ยืนยันส่งคำตอบนี้ถึงเจ้าของเว็บไซต์?")) return; setQuestionStatus("กำลังส่งคำตอบ…"); submitLetterResponse.mutate({ slug, answer: questionAnswer.trim(), startedAt: questionOpenedAt.current, honeypot: questionHoneypot }); };
  ...
  return <div className={`legacy-anniversary min-h-screen legacy-bg-${features.backgroundStyle} legacy-visual-${visualTheme.id} ${customFontFace ? "legacy-font-custom" : `legacy-font-${features.fontFamily}`} ${nightMode ? "legacy-night" : ""}`} style={themeStyle}>
    ...
    {unlocked && <main className="legacy-main-content">
      ...
      {features.questionLetterEnabled && features.questionLetterPrompt && <section className="legacy-section legacy-question-letter-section"><button type="button" className="legacy-question-letter" onClick={openQuestionLetter}><span className="legacy-letter-seal"><MailOpen size={20} /></span><span className="legacy-letter-copy"><strong>{features.questionLetterTitle || "คำถามถึงเธอ"}</strong><span>มีคำถามอยากให้ตอบ กดเพื่อเปิดจดหมาย</span></span></button></section>}
    ...
    {unlocked && questionOpened && <section className="legacy-question-modal" role="dialog" aria-modal="true" aria-label="จดหมายคำถาม"><button type="button" className="legacy-memory-modal-backdrop" onClick={() => setQuestionOpened(false)} aria-label="ปิดจดหมายคำถาม" /><article><button type="button" className="legacy-memory-modal-close" onClick={() => setQuestionOpened(false)} aria-label="ปิด"><X size={19} /></button><MailOpen size={27} /><p className="legacy-question-eyebrow">A little question for you</p><h3>{features.questionLetterTitle || "คำถามถึงเธอ"}</h3><p>{features.questionLetterPrompt}</p><textarea value={questionAnswer} onChange={(event) => setQuestionAnswer(event.target.value)} placeholder="พิมพ์คำตอบของเธอตรงนี้…" rows={5} maxLength={2000} /><input className="legacy-question-honeypot" tabIndex={-1} autoComplete="off" value={questionHoneypot} onChange={(event) => setQuestionHoneypot(event.target.value)} aria-hidden="true" /><button type="button" disabled={submitLetterResponse.isPending || !questionAnswer.trim()} onClick={submitQuestion}>{submitLetterResponse.isPending ? "กำลังส่ง…" : <><Send size={16} />ส่งคำตอบ</>}</button>{questionStatus && <small role="status">{questionStatus}</small>}</article></section>}
